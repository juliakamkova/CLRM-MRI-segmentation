...
class Framework(pl.LightningModule):
    def __init__(self, net, params, n_cl):
        ...
        self.dice_metric = DiceMetric(include_background=False, reduction="mean", get_not_nans=False, num_classes = 3)
    ...
    def train_dataloader(self):
        self.training_dataloader = torch.utils.data.DataLoader(self.training_ds, **self.params['train']['loader'],  num_workers=4, persistent_workers=True) #excessive worker creation might get DataLoader running slow or even freeze
        return self.training_dataloader

    def val_dataloader(self):
        if not hasattr(self, 'validation_ds'):
            print(self.__dict__)
            raise AttributeError("Validation dataset is not defined!")
            exit()
        self.validation_dataloader = torch.utils.data.DataLoader(self.validation_ds, **self.params['valid']['loader'], num_workers=4, persistent_workers=True) #excessive worker creation might get DataLoader running slow or even freeze
    ...
    def training_step(self, batch, batch_idx):
        ...
        print ('batch training loss and Dice', loss, train_monai_dice)
    ...
    def on_validation_epoch_end(self):
        ...
        print('checking number of val batches: ', num_items)
